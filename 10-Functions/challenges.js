...
const poll = {
  ...
  registerNewAnswer() {
    ...
    if (answer <= 3 && answer >= 0) {
      ...
      alert("Wrong input... Write option number correctly..");
      this.displayResults();
    }
  },
  displayResults(type = "array", points = this.answers) {
    if (type === "string") {
      const output = `Poll results are ${points.join(",")}`;
      console.log(output);
    } else if (type === "array") {
      console.log(points);
    } else {
...
const testDisplayResults = poll.displayResults;

testDisplayResults.call(testData);
testDisplayResults.call(null, "string", [1, 5, 3, 9, 6, 1]);
